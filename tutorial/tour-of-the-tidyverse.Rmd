...
```{r}
knitr::opts_chunk$set(message = FALSE, warning = FALSE, collapse = TRUE, include = TRUE)
```
...
## Option 1: group_by() and summarize()
...
Likewise, we can use related function `add_count()` along with `mutate()` to add up the counts per species group to use as a denominator ("count_species") when we calculate the proportion by sex.

```{r count-mutate}
penguins %>% 
  count(species, sex) %>%
  add_count(species, wt = n, name = "count_species") %>%
  mutate(prop = n/count_species*100)
```
...
> Note: Not all colors will show well, like `polarnight` below. `prismatic::color()` relies on a package that kinda has limited functionality in this sense (`crayon`). It's doing it's best :)
